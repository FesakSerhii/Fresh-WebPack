...
class VintageFrontend extends Generator {
    ...
    prompting() {
        this.log(yosay(
            `FreshWeb Production ${chalk.yellow('fresh-web-frontend')} generator`
        ));
        ...
        return this.prompt([
            {
                ...
                validate: name => {
                    if (!/\w+/.test(name)) {
                        return 'Название проекта должно состоять только из 0~9, a~z, A~Z, _, .';
                    }
            ...
            },
            {
                type: 'list',
                name: 'templateEngine',
                message: 'Какой механизм шаблонов вы предпочитаете?',
                default: 'mustache',
                choices: [
                    {
                        name: 'pug',
            ...
            },
            {
                type: 'confirm',
                name: 'jquery',
                message: 'Ваш проект будет включать jQuery?',
                default: true
            },
            {
                type: 'confirm',
                name: 'install',
                message: 'Установите зависимости прямо сейчас?',
                default: true
            }
        ])
    ...
    configuring() {
        ...
        fs.exists(this.destinationPath(this.answers.name), exists => {
            if (exists && fs.statSync(this.destinationPath(this.answers.name)).isDirectory()) {
                this.log.error(`Каталог [${this.answers.name}] существует`);
                process.exit(1);
            }
    ...
    writing() {
        const props = {
            ...
            isPug: this.answers.templateEngine === 'mustache'
        };
        ...
        // копировать исходный каталог
        template('src', 'src');

        // копировать каталог
        template('www', 'www');

        // создавать папки для изображений, шрифтов, скриптов
        ...
        try {
            ...
            console.error(`Ошибка при создании каталогов шаблонов: ${e.message}`)
        }
    ...
    install() {
        ...
        this.log(chalk.green(`\n\nСгенерирован проект '${this.answers.name}'. Удачи Вам!\n`));
    }
}
